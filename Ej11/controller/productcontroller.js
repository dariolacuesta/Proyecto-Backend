...
const containerProducts = new ContainerApi(config.mariaDB, "ecommerceproducts");
...
class Product {
	async getQuery(req, res) {
		...
		else res.status(200).json({ success: true, result: products });
	}
	...
	async put(req, res) {
		...
		if (update)
			return res.status(200).json({ success: true, result: "Product updated" });
